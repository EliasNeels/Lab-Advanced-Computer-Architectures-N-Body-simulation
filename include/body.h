...
inline void bodiesFreeDevice(Bodies& b) {
    if(b.pos_x) cudaFree(b.pos_x); if(b.pos_y) cudaFree(b.pos_y);
    if(b.vel_x) cudaFree(b.vel_x); if(b.vel_y) cudaFree(b.vel_y);
    if(b.acc_x) cudaFree(b.acc_x); if(b.acc_y) cudaFree(b.acc_y);
...
inline void bodiesUpload(Bodies& b, const BodyHost* hostBodies, int n) {
    ...
    free(hpx); free(hpy); free(hvx); free(hvy); free(hm); free(hr);
    b.count += n;
}